...
def model_predictions(
    ...
    average_scores = _average_fold_predictions(fold_0_scores_loc, fold_1_scores_loc, fold_2_scores_loc, fold_3_scores_loc, fold_4_scores_loc)
...
def _average_fold_predictions(
    ...
    for col_name in score_dfs[0].columns:
        ...
        # p-value columns/scores are geometrically averaged
            ...
            average_df_columns.append(reduce(operator.mul, df_cols) ** (1/len(score_dfs)))
    average_df = pd.concat(average_df_columns, axis=1)
    average_df.columns = score_dfs[0].columns
    return average_df

def _compute_in_peaks(average_scores: pd.DataFrame, peaks_dnatree_loc: str) -> List[bool]:
...
def _parse_args():
    parser = argparse.ArgumentParser(description="Average fold-level scores to compute model-level variant scores, including whether variants are in model peaks.")
    parser.add_argument("-f0", "--fold_0_scores_loc", required=True, type=str, help="The path to the scores generated from fold 0 of the model.")
    parser.add_argument("-f1", "--fold_1_scores_loc", required=True, type=str, help="The path to the scores generated from fold 1 of the model.")
    parser.add_argument("-f2", "--fold_2_scores_loc", required=True, type=str, help="The path to the scores generated from fold 2 of the model.")
    parser.add_argument("-f3", "--fold_3_scores_loc", required=True, type=str, help="The path to the scores generated from fold 3 of the model.")
    parser.add_argument("-f4", "--fold_4_scores_loc", required=True, type=str, help="The path to the scores generated from fold 4 of the model.")
    parser.add_argument("-p", "--peaks_dnatree_loc", required=True, type=str, help="The path to the model's peaks DNATree.")
    parser.add_argument("-o", "--output_path", required=True, type=str, help="The path to save the final model predictions.")
    return parser.parse_args()
  
if __name__ == "__main__":
    args = _parse_args()
    model_predictions(args.fold_0_scores_loc, args.fold_1_scores_loc, args.fold_2_scores_loc, args.fold_3_scores_loc, args.fold_4_scores_loc, args.peaks_dnatree_loc, args.output_path)